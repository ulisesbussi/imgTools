...
class ModeloCoordenadas(Modelo):
	...
	def __init__(self,x=[0,0,0,0],pars=None):
	...
	def createCameraMat(self,pars):
		self.M = np.array([	[pars[0],  0    , -pars[2],0],
							[  0    ,pars[1], -pars[3],0],
							[  0    ,  0    ,  1      ,0],
							[  0    ,  0    ,  0      ,1]])
	...
	def getFu(self,X=None,u=None):
		...
		u00 = z*x1
		u02 = - x1*((cx*x0 + cy*x2)*z + fx*x*x0 + fy*y*x2)
		u11 = z*x3
		u12 =   x3*((cx*x2 - cy*x0)*z + fx*x*x2 - fy*y*x0)

		Fu = np.array( [[u00,0,u02,0], [0,u11,u12,0], [0,0,0,1]]).T
		return Fu
...
class Gps_aXYZ(Modelo):
	...
	def latlonTodeltas(self,ref=None,med=None):
		...
		delta = med-ref
		deltaLat,deltaLon = delta[0:2]#esto lo cambieee
		mlat = (med+ref)[1]/2
	...
	def get_xyz(self,X=None,oldMed=None,med=None):
		...
		xn =  dx#cd * X[0] + sd *X[1] + c * dx - s * dy
		yn = dy#-sd * X[0] + cd *X[1] + s * dx - c * dy
		zn =  dz#X[2]  + dz
		Xnew = np.array([xn,yn,zn,-th])
		return Xnew
